...
gives a median pointwise $4.54\times$ gain in minimum singular value and a
median pointwise $2.28\times$ improvement in Jacobian condition number. In
grouped physical interpolation,
...
We ask whether physically computed photon-geometric observables improve the
identifiability and out-of-sample recovery of $(k,\wq)$, and whether that
conclusion survives numerical-resolution tests. We answer this by combining
validated timelike and null Kiselev geodesics with Jacobian diagnostics,
leakage-aware random, grouped, and directional evaluation, non-learned inverse
baselines, and frozen-estimator resolution audits.
...
\end{align}
Here $k$ controls the deformation strength and $\wq$ its radial dependence; we
use geometrized units with $M=1$. Leading-eikonal ringdown features use the
unstable circular null orbit, with the limitations of the geodesic--QNM
correspondence kept explicit
\cite{cardoso2009geodesic,konoplya2017correspondence,palominoylla2026ringdown}.

The mature physical study evaluates an $11\times11$ registered grid: 121
systems with $k\in[0,0.0025]$ and
$\wq\in[-0.7125,-0.45]$. The $k=0$ systems are retained deliberately because
their Schwarzschild metric is independent of $\wq$ and therefore supplies the
exact structural non-identifiability boundary. Each grid point denotes one
physical system; its full phase-resolved simulation is later compressed into
one feature vector.
...
practical recoverability at finite precision. Exact $k=0$ rank loss is
structural, while small nonzero $k$ can be practically ill-conditioned.
Historical synthetic and proxy results are kept separate in
Appendix~\ref{app:historical}. Having defined the inverse problem and its exact
rank-deficient boundary, we next construct the physical observables used to
evaluate it.

\section{Physical Geodesic Shooting and Numerical Validation}
For each registered pair $(k,\wq)$, we construct the Kiselev spacetime and
integrate one timelike emitter orbit with $M=1$, $r_p=8M$, $r_a=12M$, and
physical coordinate azimuth $\phi\in[\pi,3\pi]$. The emitter starts at
apocentre at $\phi=\pi$. We then select a finite set of emission phases. At
each phase we read the emitter state, solve the two launch angles of the direct
photon branch, integrate the null ray to the observer at $(0,0,-80M)$, and
project its momentum into the static observer tetrad. The nominal feature
archive uses 81 phase samples per system; the uniform 161-phase and targeted
321-phase calculations are numerical audits discussed later, not replacements
for the nominal archive.
...
The implementation is designed to report any failed phase rather than
interpolate or replace it with a synthetic geodesic proxy. In the accepted
121-system science grid, the required shooting-success fraction is unity.
Photon momenta are projected onto the
static observer tetrad before sky angles are formed. At every successful phase
we record redshift, impact parameter, signed
observer-tetrad sky coordinates, excess delay, and relative arrival time.
Redshift uses
...
An independent implementation cross-checked the emitter and photon dynamics,
observer-frame observables, extracted features, and selected Jacobians without
importing production \texttt{bhhairml} physics code. The maximum
trajectory/observable, full feature-vector, and selected-Jacobian absolute
differences were $7.63\times10^{-9}$, $7.77\times10^{-11}$, and
$2.62\times10^{-13}$, respectively.
...
\begin{figure}[tbp]
\centering\includegraphics[width=.78\textwidth]{shooting/phase_coloured_photon_shooting_horizontal.pdf}
...
Figure~\ref{fig:shooting-observables} shows examples of the raw observables
recorded at the selected phases for three validated spacetimes.

\begin{figure}[tbp]
\centering\includegraphics[width=.88\textwidth]{shooting/shooting_observables_vs_phase_refined.pdf}
...
amplitude and independent parameter information are not equivalent. Markers
highlight directly simulated phase samples; connecting lines are visual guides
only and do not represent interpolated physical values.}
\label{fig:shooting-observables}
\end{figure}

\FloatBarrier

These discrete phase-resolved samples are the quantities subsequently
compressed into fixed-dimensional feature vectors.

\subsection{From phase-resolved observables to feature vectors}
For one physical system and one raw observable, the solver returns the finite
sequence $O(\phi_1),O(\phi_2),\ldots,O(\phi_N)$ at the selected emission
phases. These are directly simulated samples, not an interpolated continuous
curve. We summarize each sampled phase series by its mean, amplitude,
reference value where defined, and sine/cosine coefficients through the third
harmonic. The Fourier coefficients compress the successful computed samples;
they are never used to fill a failed phase.

The registered families are defined before any comparison: \emph{ringdown}
contains four leading-eikonal scalar features; \emph{orbital} contains the
radial azimuthal period, apsidal advance, and summaries of $r$ and $p_r$ (20
features); \emph{photon geometry} summarizes impact parameter and the two
signed observer-tetrad sky coordinates (27); \emph{redshift} contains 9
summaries; and \emph{timing} summarizes excess delay and relative arrival time
(17). \emph{All shooting} is orbital plus photon geometry, redshift, and
timing (73). Ringdown plus photon geometry has 31 features, and ringdown plus
all shooting has 77. Appendix~\ref{app:features} gives the exact registered
table.
...
Large response is insufficient when the two parameter-response directions are
nearly collinear. In Fig.~\ref{fig:local}, $\sigma_{\max}$ measures the
strongest response, $\smin$ the weakest identifiable direction, their angle
measures directional separation, and $\kappaJ=\sigma_{\max}/\smin$ measures
conditioning. The finite-difference diagnostic is anchored at
$(k,\wq)=(10^{-3},-0.5)$: the $k$ response uses the step from Schwarzschild to
that anchor, while the $\wq$ response uses $-0.5\to-2/3$ at fixed $k=10^{-3}$.

\begin{figure}[tbp]
\centering\includegraphics[width=.78\textwidth]{physical_local_sensitivity_comparison.pdf}
\caption{Representative finite-difference response and separation diagnostics anchored at
$(k,\wq)=(10^{-3},-0.5)$ with the finite-difference stencil described in the
text. Timing has a
...
number to 81.4. Higher $\sigma_{\min}$, angles farther from $0^\circ$ or
$180^\circ$, and lower condition number are favorable; magnitude alone is not
identifiability.}
\label{fig:local}
\end{figure}

The full observer tracks nearly overlap for these small deformations, but their
matched-phase differences are numerically resolved; the actual-scale tracks
and residuals are provided in Appendix~\ref{app:numerical-thresholds}.

Using the nominal 81-phase Jacobian archive across the accepted physical grid,
ringdown plus photon geometry gives a median pointwise $\smin$ gain of
$4.5429\times$ and a median pointwise condition-number improvement of
$2.2819\times$ relative to ringdown alone. These are medians of pointwise
improvement ratios, not ratios of the displayed grid medians. The grid medians
for the combined set are
...
Does the representative complementarity seen at the reference point persist across the
physical parameter grid? Figure~\ref{fig:complementarity} gives the primary
grid-wide comparison. The pointwise maps show that complementarity varies
across the finite-$k$ domain; the full maps are provided in
Appendix~\ref{app:grid-maps}.

\begin{figure}[tbp]
\centering\includegraphics[width=.74\textwidth]{physical_observable_complementarity_landscape.pdf}
\caption{Nominal 81-phase grid-wide summaries of $\smin$ and condition number:
each dot is the grid median and each bar spans the 25th--75th percentile across
physical grid points, not measurement uncertainty. Higher $\smin$ and lower
condition number are favorable. The separate 161-phase audit is reported in
the text.}
...
\begin{tabular}{lcc}
Feature set & med. $\smin$ (higher better) & med. $\kappaJ$ (lower better) \\
\hline
Ringdown & 0.2782 & 6.6112 \\
Rdown + photon geom. & 1.2326 & 4.0731 \\
...
Because local Jacobian rank cannot rule out distant observable near-collisions,
we also checked the sampled physical grid. No unresolved distant finite-$k$
observable collision was found on the 121-point grid at the tested numerical
resolution, and photon geometry improves the distant-pair separation
diagnostics. This finite-grid check does not prove continuous global
injectivity; the complete thresholds and neighbor diagnostics appear in
Appendix~\ref{app:ambiguity}.

\section{Leakage-Aware Inverse-Learning Protocols}
Each physical system contributes one supervised inference sample: a single
feature vector $\mathbf{x}$ constructed from its entire phase-resolved
simulation, with target $(k,\wq)$. Individual phase samples are not treated as
independent training examples. Training, calibration, and test partitions are
therefore formed at the physical-system level.
...
preferable.
The increasingly demanding protocols are random interpolation, two blocked
$3\times3$ grouped physical layouts, and four directional extrapolation tests.
All nominal $k=0$ records
...
Figure~\ref{fig:protocols} asks how recoverability changes as validation moves
from random interpolation to grouped physical holdouts and directional
extrapolation.

\begin{figure}[tbp]
\centering\includegraphics[width=.84\textwidth]{physical_ml_protocol_comparison.png}
\caption{NMAE for the five primary feature sets under random, grouped physical
interpolation, and directional extrapolation. Each dot preserves the registered
table statistic: the median of archived summary-row medians. To expose
lower-level dispersion rather than suppress it through that hierarchy, each
horizontal bar spans the 25th--75th percentile of the underlying NMAE records
across the applicable folds, seeds, models, and, for extrapolation, directions.
Thus the point and bar intentionally summarize different aggregation levels;
directions remain separate in the machine-readable source before aggregation.}
\label{fig:protocols}
\end{figure}

\subsection{Traditional inverse baselines}
To test whether the photon-geometry benefit belongs to the physical forward
map rather than to one ML architecture, we evaluate non-learned nearest-model
and local Jacobian inversions under exactly the same registered physical
...
\begin{figure}[tbp]
\centering\includegraphics[width=.95\textwidth]{traditional_vs_ml_inverse.pdf}
...
\begin{figure}[tbp]
\centering\includegraphics[width=.82\textwidth]{physical_ml_uncertainty_calibration.png}
...
We first ask whether the physical forward calculation converges as phase
resolution increases. Figure~\ref{fig:resolution} addresses that question.
...
\centering\includegraphics[width=.91\linewidth]{resolution_robustness_compact.pdf}
\captionof{figure}{Targeted pointwise maximum normalized feature changes from 81 to 161
...
We next ask whether the previously trained estimators remain stable after the
forward map has converged. Figure~\ref{fig:estimator-robustness} applies those
frozen estimators to the refined features; no model is retrained.
...
iteration limit, and no numerical overflow is detected. Thus, the forward map
satisfies the convergence criterion, whereas the frozen estimators do not
satisfy all robustness criteria.
...
\begin{figure}[tbp]
\centering\includegraphics[width=.78\textwidth]{targeted_estimator_robustness_compact.pdf}
...
shift for HGB, RF, and MLP. The annotation summarizes the corresponding MLP
failure diagnostics. No predictions are clipped.}
...
tree tail sensitivity persists, and MLP extrapolation is unstable. These
limitations preclude interpreting the results as an observational constraint.
...
\centering
\includegraphics[width=.93\linewidth]{historical_analytic_identifiability_lens.pdf}
\captionof{figure}{Historical analytic identifiability lens. Left: local Jacobian
condition number, which grows near $k=0$. Center: minimum singular value,
showing the weakest identifiable direction. Right: change in identifiable-$\wq$
error after adding historical synthetic geodesic proxies; the mean MAE changes
from 0.0225 to 0.0143. White regions are invalid or missing source-grid points
and are not interpolated. The third panel is not physical ray tracing, and
proxy augmentation does not resolve exact $k=0$ rank loss.}
\label{fig:lens}
\end{minipage}
\end{center}
...
\centering\includegraphics[width=.88\linewidth]{learning_when_hair_is_observable_rank_aware.pdf}
\captionof{figure}{Historical dense synthetic rank-aware identifiability boundary. Exact
...
\section{Pointwise Finite-Grid Identifiability Maps}
\label{app:grid-maps}
The grid-wide medians and interquartile ranges in the main text summarize the
overall observable-family ordering. Figure~\ref{fig:grid} retains the
corresponding pointwise structure: it shows where local identifiability is
strong or weak within the registered domain without interpolating missing or
invalid systems.

\begin{center}
\begin{minipage}{.98\textwidth}
\centering\includegraphics[width=.97\linewidth]{physical_grid_minimum_singular_value.png}
\captionof{figure}{Grid maps of $\log_{10}\smin$. Crosses at $k=0$ mark exact structural
rank loss rather than missing simulations. Missing or invalid values are not
interpolated, and all panels use one finite-$k$ color scale.}
\label{fig:grid}
\end{minipage}
\end{center}

\section{Additional Inverse-Learning Diagnostics}
\subsection{Additional model-level results}
...
\subsection{Four directional extrapolation tests}
...
\subsection{Conformal calibration and rejection}
...
\subsection{Noise and learning curves}
...
\centering\includegraphics[width=.92\linewidth]{conditioning_vs_wq_recovery.pdf}
...
\subsection{Numerical-validation thresholds}
...
\centering
\includegraphics[width=.86\linewidth]{schwarzschild_arrival_validation_with_residuals.pdf}
\captionof{figure}{Schwarzschild arrival-time validation. Four arrival-time phase-resolved series are
...
For the small-deformation comparison, the full observer tracks nearly overlap
on their physical scale. Figure~\ref{fig:sky-residuals} separates those tracks
from the small but resolved matched-phase residuals.

\begin{center}
\begin{minipage}{.88\textwidth}
\centering\includegraphics[width=.89\linewidth]{shooting/observer_sky_track_with_residuals.pdf}
\captionof{figure}{Static-observer-tetrad sky tracks and actual-scale residuals for the
...
\centering\includegraphics[width=.96\linewidth]{shooting/schwarzschild_kiselev_ray_comparison.pdf}
...
\subsection{Uniform 161-phase convergence audit}
...
inverse numerical audit passed. The independent physics cross-check reported
in Section~3 also passed; full implementation and provenance details remain in
the repository audit files.
...
Numerically, the closest distant-pair distance increases from 0.02517 to
0.03155 against a maximum measured forward-resolution change of 0.002462;
the number of distant pairs below local-median spacing falls from 13 to 6, and
the maximum nearest-observable-neighbor parameter distance changes from
0.23216 to 0.21886. Photon geometry does not preserve every local
nearest-neighbor ordering even though it improves the distant-pair diagnostics.
