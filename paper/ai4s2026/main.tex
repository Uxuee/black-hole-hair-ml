...
an observational constraint. Final inverse validation confirms lower grouped
$w_q$ error for the combined set in both tree families, but directional
extrapolation is substantially harder and 81-to-161-phase feature substitution
changes selected tree predictions materially. The result therefore establishes
physical complementarity while identifying a remaining numerical-robustness
condition for final inverse-model claims.
\end{abstract}
...
Figure~\ref{fig:features} gives the best-per-target feature comparison.
...
Figure~\ref{fig:observable} compares the analytic identifiability class with
the grouped-CV classifier prediction.
...
Figure~\ref{fig:lens} provides the companion proxy-identifiability lens.
...
$10.28431$, giving physical apsidal advance $4.00113$. Figure~\ref{fig:physical-schwarzschild}
compares the direct and independently integrated-redshift arrival curves.
...
signal significance. Figures~\ref{fig:physical-small-k} and
\ref{fig:physical-wq} show the corresponding numerical signal and fixed-$k$
sensitivity checks.
...
Figure~\ref{fig:physical-local} compares the observable groups directly.
...
$-0.7125\leq\wq\leq-0.45$. Figure~\ref{fig:physical-grid-sigma} shows the
grid-wide minimum-singular-value variation.

At $k=0$, the metric is independent of $\wq$ and
\begin{equation}
 \frac{\partial\mathbf O}{\partial\wq}=0,
\end{equation}
so exact rank loss is required physically, as shown in
Fig.~\ref{fig:physical-kzero}.
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.98\textwidth]{physical_grid_minimum_singular_value.png}
\caption{Base-10 logarithm of the minimum singular value for each predeclared
physical observable set. Exact $k=0$ rank-loss points are masked in grey, and
the shared colour scale is computed only from finite $k>0$ values. Each square
is a computed grid point; missing or invalid points are not interpolated.}
\label{fig:physical-grid-sigma}
\end{figure*}

Twenty-seven 161-phase checks change
...
the gain. Figure~\ref{fig:physical-complementarity} summarizes the grid-wide
median and interquartile ranges.

\begin{figure*}[!t]
\centering
\includegraphics[width=0.98\textwidth]{physical_observable_complementarity.png}
\caption{Grid-wide median and interquartile range of the minimum singular value
(left; higher is better) and condition number (right; lower is better), using
finite $k>0$ points. Full names identify every observable construction.}
\label{fig:physical-complementarity}
\end{figure*}

\FloatBarrier
\section{Inverse Learning from Physical Shooting Features}
We finally test whether the forward Jacobian predicts held-out inverse-model
behavior. The 121-point table is evaluated with histogram gradient boosting,
random forest, and a small target-scaled MLP over five deterministic seeds.
Random interpolation, two contiguous 3-by-3 block layouts, and four directional
extrapolation bands are kept separate. At $k=0$, $\wq$ is retained for boundary
diagnostics but excluded from ordinary $\wq$ scores because it is exactly
unobservable.

\begin{table}[t]
\caption{Median normalized MAE across models, folds, and seeds.}
\label{tab:physical-ml}
\centering
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lcccccc}
\toprule
Features & \multicolumn{2}{c}{Random} & \multicolumn{2}{c}{Grouped} & \multicolumn{2}{c}{Directional}\\
 & $k$ & $\wq$ & $k$ & $\wq$ & $k$ & $\wq$\\
\midrule
Ringdown & .0361 & .1688 & .0485 & .3093 & .0796 & .3011\\
Photon geometry & .1088 & .0927 & .1813 & .1111 & .2291 & .2224\\
All shooting & .0750 & .0453 & .1241 & .0729 & .2305 & .2072\\
Ringdown+photon & .0378 & .0730 & .0457 & .0874 & .1305 & .2012\\
Ringdown+all shooting & .0427 & .0454 & .0445 & .0727 & .1363 & .2067\\
\bottomrule
\end{tabular}
}
\end{table}

Random splitting is optimistic: grouped/random degradation reaches 1.67 for
$k$ and 1.83 for identifiable $\wq$. Ringdown alone is strong for $k$ but poor
for $\wq$; adding photon geometry reduces grouped $\wq$ NMAE from 0.3093 to
0.0874. HGB and random forest both support this complementarity, while the MLP
and individual extrapolation directions provide counterexamples to universal
improvement. Aggregate directional ringdown+photon NMAE is 0.1305 for $k$ and
0.2012 for $\wq$. Figure~\ref{fig:physical-ml-protocol} compares the validation
protocols, and Fig.~\ref{fig:physical-ml-jacobian} aligns grouped inverse error
with the conditioning statistics.

\begin{figure*}[!t]
\centering
\includegraphics[width=0.98\textwidth]{physical_ml_protocol_comparison.png}
\caption{Median normalized MAE for $k$ (left) and identifiable $\wq$ (right)
under random interpolation, held-out contiguous regions, and directional
extrapolation. Central values reproduce Table~\ref{tab:physical-ml} exactly;
intervals show fold/seed/model dispersion and, for the extrapolation summary,
the four separately archived directions.}
\label{fig:physical-ml-protocol}
\end{figure*}

\begin{figure*}[t]
\centering
\includegraphics[width=0.99\textwidth]{physical_ml_vs_jacobian_complementarity.png}
\caption{Physical conditioning and grouped inverse-learning performance in a
common feature-set order. Panels show (A) median minimum singular value (higher
is better), (B) median condition number (lower is better), (C) grouped
identifiable-$\wq$ NMAE, and (D) grouped-$k$ NMAE for HGB, RF, and MLP (lower is
better). Physical conditioning generally agrees with tree-based grouped $\wq$
performance, but does not perfectly rank every model, target, or protocol.}
\label{fig:physical-ml-jacobian}
\end{figure*}

In a pooled explanatory model, standardized $\log\kappa$ has coefficient
0.197 with 95\% bootstrap interval [0.185,0.210], after including training
distance (0.330 [0.317,0.342]) and extrapolation status
(0.276 [0.263,0.287]); $R^2=0.144$. By contrast, the coefficient of
$-\log\sigma_{\min}$ is consistent with zero. Conditioning remains informative,
but coverage is stronger and no causal interpretation is claimed.

Split-conformal intervals target 90\% coverage. Empirical $k/\wq$ coverage is
89.2\%/96.5\% for random splits, 72.2\%/85.1\% for grouped regions, and
50.9\%/66.0\% for extrapolation. Formal exchangeability guarantees therefore
do not extend to these shifted tests. Learning curves are still improving at
78 training points, and grouped one-percent feature noise gives
ringdown+photon NMAE 0.0699/0.0853. Figure~\ref{fig:physical-ml-uncertainty}
shows the empirical coverage and its dispersion.

\begin{figure*}[t]
\centering
\includegraphics[width=0.92\textwidth]{physical_ml_uncertainty_calibration.png}
\caption{Empirical split-conformal coverage for $k$ and identifiable $\wq$
(with $k=0$ excluded from ordinary $\wq$ coverage), separated by validation
protocol. Points and intervals are the median and interquartile range across
models (and directions where applicable); the dashed line is nominal 0.90
coverage. Extrapolation coverage is empirical and is not guaranteed under
distribution shift.}
\label{fig:physical-ml-uncertainty}
\end{figure*}

The principal failed check is phase-resolution robustness. On 27 independently
recomputed points, replacing 81-phase shooting features with 161-phase values
changes selected HGB predictions by up to 0.000614 in $k$, approximately 0.100
in $\wq$, and 0.093 in NMAE. The physical Jacobian is converged, but the inverse
tree predictions are not uniformly robust to these small feature perturbations,
as summarized in Fig.~\ref{fig:physical-ml-highres}.

A subsequent frozen-protocol replication regenerated all 121 physical points
uniformly at 161 phases. All physical checks pass, and the median normalized
feature difference is $7.91\times10^{-4}$, but 240 of 9317 feature--point
comparisons exceed the predeclared 0.05 convergence threshold, led by
third-harmonic timing and orbital coefficients. The median scored normalized
prediction shift is $9.48\times10^{-4}$ (95th percentile 0.0261), yet
high-$\wq$-to-low-$\wq$ MLP extrapolations with all shooting features are
catastrophically unstable: the largest aggregate median-NMAE change is 16.85.
Thus uniform sampling removes mixed resolution but does not resolve inverse
robustness; the journal-readiness verdict remains conditional.

A predeclared targeted 321-phase audit then recomputed 35 scientifically
informative points without interpolation or proxy replacement. All points pass
the physical checks. The median and 95th-percentile normalized 161-to-321
feature changes fall to $1.96\times10^{-4}$ and $7.23\times10^{-3}$,
respectively, with no unresolved comparisons and unchanged Jacobian rank.
Nevertheless, the frozen HGB/RF 95th-percentile normalized prediction shifts
are 0.0812/0.0851 and their largest identifiable-$\wq$ shifts are
0.306/0.271. The predeclared decision is therefore \textbf{Outcome B}:
forward features converge, but the tree families remain sensitive in their
tails. A fixed Extra Trees baseline and training-fold numerical-perturbation
ensembles do not pass all robustness and grouped-performance criteria.
Catastrophic MLP errors remain un-clipped, but 409 of 410 audited records are
already catastrophic at 161 phases and none is materially worsened by the
321-phase substitution. Journal readiness consequently remains conditional
because estimator robustness, rather than forward convergence, is the
outstanding blocker.

\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth]{physical_ml_high_resolution_robustness.png}
\caption{High-resolution robustness when 161-phase shooting features replace
81-phase features at 27 validation points. Panel A reports the maximum
individual prediction shift normalized by the target range; panel B reports
the absolute change in aggregate NMAE. Targets are shown separately, and the
ringdown-only construction is unchanged because it contains no shooting
features.}
\label{fig:physical-ml-highres}
\end{figure*}
...
physical feature grid is modest. Random, blocked-region, directional,
conformal, and feature-noise tests are now reported, but learning curves remain
data limited and shifted conformal coverage is below nominal. The uniform
161-phase rerun retains unresolved third-harmonic feature differences, while
the targeted 321-phase audit resolves those forward-feature comparisons but
shows that HGB/RF tail sensitivity still exceeds the frozen criteria.
Catastrophic MLP extrapolation tails remain a separate estimator failure. These
issues must be resolved before final inverse-performance claims;
...
complementary ringdown and photon-geometry observables improve conditioning and
grouped $\wq$ inference in both tree families. Random validation is optimistic,
directional extrapolation is harder, and conformal uncertainty does not fully
recognize shift. The high-resolution substitution test prevents a claim of
fully converged inverse performance. The broader AI-for-science principle is
that inverse models should learn when parameters are observable---and expose
when numerical or coverage limits invalidate a confident fit.
